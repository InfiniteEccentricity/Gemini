...
def process_cmd(yaml_file, local=False):
    ...
    if use_container == "docker":
        ...
        ps_cmd = f"{sys.executable} {yaml_conf['exp_path']}/{yaml_conf['aggregator_entry']} {conf_script} --this_rank=0 --num_executors={total_gpu_processes} --executor_configs={executor_configs} "
        ...
        for cuda_id in range(len(gpu)):
            for _ in range(gpu[cuda_id]):
                if use_container == "docker":
                    ...
                    worker_cmd = f"{sys.executable} {yaml_conf['exp_path']}/{yaml_conf['executor_entry']} {conf_script} --this_rank={rank_id} --num_executors={total_gpu_processes} --cuda_device=cuda:{cuda_id} "
                rank_id += 1

...
def init_dataset():
    ...
    if not os.path.exists(parser.args.data_dir):
        os.makedirs(parser.args.data_dir, exist_ok=True)
    
    # Create a lock file to prevent concurrent downloads
    lock_path = os.path.join(parser.args.data_dir, "download.lock")

    with open(lock_path, "w") as lock_file:
        # Acquire exclusive lock (blocks other executors until released)

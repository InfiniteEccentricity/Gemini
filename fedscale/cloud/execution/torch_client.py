...
class TorchClient(ClientBase):
    ...
    def train(self, client_data, model, conf):
        ...
        if getattr(conf, 'local_epochs', None):
            ...
            logging.info(f"Client {client_id}: Training for {conf.local_epochs} epochs ({target_steps} steps)")
        ...
        results = {'client_id': client_id, 'moving_loss': self.epoch_train_loss,
                   'trained_size': self.total_samples_processed, # self.completed_steps * conf.batch_size,
    ...
    def get_optimizer(self, model, conf):
        ...
        if conf.task == "detection":
            ...
            optimizer = torch.optim.SGD(
                model.parameters(), lr=conf.learning_rate, momentum=conf.client_momentum, weight_decay=conf.weight_decay)
        return optimizer
    ...
    def train_step(self, client_data, conf, model, optimizer, criterion):
        ...
        for data_pair in client_data:
            ...
            if isinstance(data, torch.Tensor):
                current_bs = data.size(0)
            elif isinstance(data, (list, tuple)):
                current_bs = len(data[0]) if len(data) > 0 else 0
            else:
                current_bs = conf.batch_size
            
            # Track actual samples processed
            self.total_samples_processed += current_bs

            # LR Norm implementation
            if conf.use_lr_norm:
                target_bs = conf.batch_size
                
                # Scale LR if batch is smaller than target
                if 0 < current_bs < target_bs:
                    logging.info(f"Batch size small ({current_bs}), using LR Norm")
                    lr_scale = current_bs / float(target_bs)
                    for param_group in optimizer.param_groups:
                        param_group['lr'] = conf.learning_rate * lr_scale
                else:
                    # Reset to original LR
                    for param_group in optimizer.param_groups:

...
class Aggregator(job_api_pb2_grpc.JobServiceServicer):
    ...
    def __init__(self, args):
        ...
        if args.wandb_token != "":
            ...
            if self.wandb.run is None:
                ...
                if self.wandb.run.sweep_id:
                    logging.info(f"Detected Sweep. Overriding config params.")
                    
                    # Get the params chosen by the Agent
                    sweep_config = self.wandb.config
                    
                    # Overwrite the args object dynamically
                    for key, value in sweep_config.items():
                        if hasattr(self.args, key):
                            logging.info(f"  -> Overriding {key}: {getattr(self.args, key)} to {value}")
                            setattr(self.args, key, value)
                            
                    # Update the WandB config display
                    self.wandb.config.update({k: v for k, v in vars(self.args).items() if k in sweep_config}, allow_val_change=True)
    ...
    def save_model(self):
        ...
        if parser.args.save_checkpoint and self.last_saved_round < self.round:
            ...
            np.save(self.temp_model_path, np.array(self.model_weights, dtype=object))
            if self.wandb != None:
